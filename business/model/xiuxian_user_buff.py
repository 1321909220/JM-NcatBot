...
# 获取用户buff列表
def get_user_buff(user_id: Optional[int] = None
...
# 给某个用户某个道具 (支持批量)
def add_user_buff(userEquipment_data: Union[Dict[str, Any], List[Dict[str, Any]]]) -> Union[XiuxianUserItem, List[XiuxianUserItem]]:
    try:
        with get_session() as session:
...
# 更新用户某buff
def update_buff_by_user_on_item(
    user_id: int,
    effect_id: int, 
    ...
    try:
        with get_session() as session:
            if use_merge:
                # 使用merge方式 (适合复杂对象)
                user = XiuxianUserItem(user_id=user_id,effect_id=effect_id,**equipment_data)
                ...
                stmt = (
                    ...
                    .where(XiuxianUserItem.effect_id==effect_id)
...
# 删除用户buff
def delete_user_buff(ids: Union[int, List[int]]) -> int:
    try:
        with get_session() as session:
...
# 删除用户某个buff记录
def delete_user_buff_by_user(user_id,effect_id)->int:
    try:
        with get_session() as session:
            stmt = delete(XiuxianUserItem).where(XiuxianUserItem.user_id==user_id).where(XiuxianUserItem.effect_id==effect_id) 
            result = session.execute(stmt) 
            session.commit() 
            return result.rowcount  
    except SQLAlchemyError as e:
